...
fn parse_deref<'a, E: ParseError<&'a str> + ContextError<&'a str>>(
    input: &'a str,
) -> IResult<&'a str, Expr, E> {
    let (input, _) = whitespace(input)?;
    let (input, _) = tag("*")(input)?;
...
fn parse_expr_struct<'a, E: ParseError<&'a str> + ContextError<&'a str>>(
    ...
    let (input, fields) = cut(separated_list0(
        ...
        alt((
            separated_pair(map(parse_symbol, Symbol::from), delimited(whitespace, char(':'), whitespace), parse_expr),
            map(parse_symbol, |s| (s.into(), Expr::Var(s.into())))
        )),
    ))(input)?;
    ...
    Ok((input, Expr::Struct(fields.into_iter().collect())))
}

fn parse_expr_enum<'a, E: ParseError<&'a str> + ContextError<&'a str>>(
    input: &'a str,
) -> IResult<&'a str, Expr, E> {
    let (input, _) = whitespace(input)?;
    let (input, ty) = parse_type(input)?;
    ...
    Ok((input, Expr::Enum(ty, name.into())))
}

fn parse_expr_union<'a, E: ParseError<&'a str> + ContextError<&'a str>>(
    input: &'a str,
) -> IResult<&'a str, Expr, E> {
    let (input, _) = whitespace(input)?;
    let (input, ty) = parse_type(input)?;
...
fn parse_mutability<'a, E: ParseError<&'a str> + ContextError<&'a str>>(
    input: &'a str,
) -> IResult<&'a str, Mutability, E> {
    let (input, _) = whitespace(input)?;
    let (input, result) = opt(tag("mut"))(input)?;
    Ok((input, result.is_some().into()))
}
...
fn parse_type_def<'a, E: ParseError<&'a str> + ContextError<&'a str>>(input: &'a str) -> IResult<&'a str, Stmt, E> {
    let (input, _) = whitespace(input)?;
    ...
    let (input, _) = whitespace(input)?;
    let (input, _) = tag(";")(input)?;
    Ok((input, Stmt::DeclareType(name.into(), ty)))
}
...
fn parse_pointer_type<'a, E: ParseError<&'a str> + ContextError<&'a str>>(input: &'a str) -> IResult<&'a str, Type, E> {
    let (input, _) = whitespace(input)?;
    let (input, _) = tag("&")(input)?;
    let (input, _) = whitespace(input)?;
    let (input, mutability) = parse_mutability(input)?;
...
pub fn parse_struct_type<'a, E: ParseError<&'a str> + ContextError<&'a str>>(input: &'a str) -> IResult<&'a str, Type, E> {
    let (input, _) = whitespace(input)?;
    let (input, _) = opt(tag("struct"))(input)?;
    let (input, _) = whitespace(input)?;
    let (input, _) = tag("{")(input)?;
    let (input, _) = whitespace(input)?;
    let (input, tys) = cut(separated_list0(
        delimited(whitespace, tag(","), whitespace),
        separated_pair(map(parse_symbol, Symbol::from), delimited(whitespace, tag(":"), whitespace), parse_type)
    ))(input)?;
    let (input, _) = whitespace(input)?;
    let (input, _) = tag("}")(input)?;
    Ok((input, Type::record(tys)))
}

fn parse_union_type<'a, E: ParseError<&'a str> + ContextError<&'a str>>(input: &'a str) -> IResult<&'a str, Type, E> {
    let (input, _) = whitespace(input)?;
    let (input, _) = opt(tag("union"))(input)?;
    let (input, _) = whitespace(input)?;
    let (input, _) = tag("{")(input)?;
    let (input, _) = whitespace(input)?;
    let (input, tys) = separated_list0(
        delimited(whitespace, tag(","), whitespace),
        separated_pair(map(parse_symbol, Symbol::from), delimited(whitespace, tag(":"), whitespace), parse_type)
    )(input)?;
    let (input, _) = whitespace(input)?;
    let (input, _) = tag("}")(input)?;
    Ok((input, Type::union(tys)))
}

fn parse_enum_type<'a, E: ParseError<&'a str> + ContextError<&'a str>>(input: &'a str) -> IResult<&'a str, Type, E> {
    let (input, _) = whitespace(input)?;
    ...
    let (input, tys) = separated_list0(
        delimited(whitespace, tag(","), whitespace),
        map(parse_symbol, Symbol::from)
    )(input)?;
    let (input, _) = whitespace(input)?;
    let (input, _) = tag("}")(input)?;
    Ok((input, Type::enum_(tys)))
}

fn parse_array_type<'a, E: ParseError<&'a str> + ContextError<&'a str>>(input: &'a str) -> IResult<&'a str, Type, E> {
    let (input, _) = whitespace(input)?;
    let (input, _) = tag("[")(input)?;
    let (input, _) = whitespace(input)?;
    let (input, ty) = parse_type(input)?;
    ...
    let (input, _) = tag("*")(input)?;
    let (input, _) = whitespace(input)?;
    let (input, size) = parse_int_literal(input)?;
    let (input, _) = whitespace(input)?;
    let (input, _) = tag("]")(input)?;
    let (input, _) = whitespace(input)?;
    Ok((input, ty.array(size as usize)))
}

fn parse_procedure_type<'a, E: ParseError<&'a str> + ContextError<&'a str>>(input: &'a str) -> IResult<&'a str, Type, E> {
    let (input, _) = whitespace(input)?;
    ...
    Ok((input, Type::procedure(args, ret_ty)))
}
...
fn parse_type<'a, E: ParseError<&'a str> + ContextError<&'a str>>(input: &'a str) -> IResult<&'a str, Type, E> {
    ...
    let (input, result) = alt((
        ...
        parse_pointer_type,
        map(parse_symbol, |name| Type::Named(name.into()))
    ))(input)?;
    let (input, _) = whitespace(input)?;
    Ok((input, result))
}
...
fn parse_proc<'a, E: ParseError<&'a str> + ContextError<&'a str>>(input: &'a str) -> IResult<&'a str, Stmt, E> {
    ...
    Ok((input, proc(name, args, ret_ty, Stmt::Block(block))))
}
...
fn parse_let<'a, E: ParseError<&'a str> + ContextError<&'a str>>(input: &'a str) -> IResult<&'a str, Stmt, E> {
    ...
    let (input, name) = parse_symbol(input)?;
    let (input, _) = whitespace(input)?;
    let (input, ty) = opt(preceded(tag(":"), parse_type))(input)?;
    let (input, _) = whitespace(input)?;
    let (input, _) = tag("=")(input)?;
    let (input, _) = whitespace(input)?;
    ...
    let (input, _) = whitespace(input)?;
    let (input, _) = tag(";")(input)?;
    Ok((input, let_var(mutability, name, ty, value)))
}
...
fn parse_let_static<'a, E: ParseError<&'a str> + ContextError<&'a str>>(input: &'a str) -> IResult<&'a str, Stmt, E> {
    ...
    let (input, _) = whitespace(input)?;
    let (input, _) = tag(";")(input)?;
    Ok((input, let_static(mutability, name, ty, value)))
}
